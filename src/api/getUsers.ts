...
function apiRequest<T>(urlPath: string): Promise<ApiResponse<T>> {
...
// get all scripts
function getAllScripts(items: FolderItem[]): FolderItem[] {
...
async function extractUsersFromScript(script: FolderItem, users: Set<string>): Promise<void> {
  try {
    const scriptResponse = await apiRequest<ScriptData>(`/api/v1/scripts/${script.id}`)
...
async function processProject(project: Project): Promise<FolderItem[]> {
  try {
    const foldersResponse = await apiRequest<{ folders: FolderItem[] }>(`/api/v1/projects/${project.id}/folders`)
...
async function getUsers(): Promise<void> {
  ...
  try {
    // Get all projects
    const projectsResponse = await apiRequest<{ projects: Project[] }>('/api/v1/projects')
