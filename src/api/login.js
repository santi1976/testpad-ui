 ...
 * @param {boolean} verbose - If true, logs progress messages (default: false)
 * @returns {Promise<{cookies: string, csrfToken: string}>} - Session cookies and CSRF token
 */
export async function login(verbose = false) {
  ...
  const loginPageResponse = await new Promise((resolve, reject) => {
  ...
  while (redirectCount < maxRedirects) {
    ...
    if (redirectResponse.status === 200) {
      ...
      const finalPageResponse = await new Promise((resolve, reject) => {
        const req = https.request({
          hostname: redirectHost,
          port: 443,
          path: redirectPath,
